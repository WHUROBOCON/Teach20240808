...
bool linkedlist_insert(LinkedList_t *list, void *data, void *place, bool head) // 插入元素,place是插入位置
{
    ...
    if (place == NULL)
    {
        ...
        else // 链表不为空,尾插
        {
            ...
            node->prev = list->tail; // 新节点的prev指针指向尾指针
...
bool linkedlist_search(LinkedList_t *list, void *data, int *result) // 查找元素
{
    ...
    return false; // 这个代码有问题吗？答案是有问题，因为result是一个指针，所以应该是*result++
}
...
int main()
{
    ...
    // int result1 = 0;
    // int *result = &result1;
    // bool search_result = linkedlist_search(&list, &b, result);
    // printf("search result: %d  ,is number %d,", search_result, *result);
    // search_result = linkedlist_search(&list, &b, result);
    // printf("search result: %d  ,is number %d,", search_result, result);
    void *data = NULL;
